...
```
sphinx-autobuild sphinx _build/html -a --open-browser --re-ignore ".*auto_examples.*"
```
...
| `--re-ignore` | `".*auto_examples.*"` | ignore all files that are auto-generated by the [`sphinx-gallery`](https://sphinx-gallery.github.io/) package (if this is not set, `sphinx-autobuild` get stuck in a loop)
